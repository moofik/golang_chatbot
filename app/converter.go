...
const MONEY_BUY_COEFFICIENT = 1.06  // 6%
const MONEY_SELL_COEFFICIENT = 0.94 // 6%
...
func ConvertCrypto(from string, to string, amount float64, buy bool) (int, int, error) {
	client := &http.Client{}
	req, err := http.NewRequest("GET", "https://api.coingecko.com/api/v3/simple/price?ids=ethereum%2Cbitcoin%2Ctether&vs_currencies=rub", nil)
	if err != nil {
		log.Print(err)
	}
